  ...
  process: function(bundlerConfig) {
    ...
    return pipeline(
      this._merge(
        pipeline(
          this._concat(config),
          stream.debug('> concat', concatStep)
        ),
        pipeline(
          this._browerify(config),
          stream.debug('> browerify', browserifyStep),
          this._map(config),
          stream.debug('> map', browserifyStep)
        )
      ),
      stream.debug('merge', step),
      this._remap(config),
      stream.debug('remap', step),
      this._uglify(),
      stream.debug('uglify', step),
      stream.debug('total', start)
    );
  },
