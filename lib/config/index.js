...
BundlerConfig.prototype = {
    ...
    return Promise
      ...
      .then(function() {
        watcher.on('update', function() {
          var start = new Date();
          logger.info('renew [%s] change detected', self.toString());
          var processStream = self.process();
          processStream
            .on('error', function(error) {
              logger.error('renew [%s]\n%s', self.toString(), error.stack);
            })
            .on('finish', function() {
              logger.info('renew [%s] cache updated in %ss', self.toString(), (new Date() - start) / 1000);
            });
        });
      });
  },
   ...
   */
  toString: function() {
    return this.get().bundleName + ':' + this.key().substr(0, 6);
  },
